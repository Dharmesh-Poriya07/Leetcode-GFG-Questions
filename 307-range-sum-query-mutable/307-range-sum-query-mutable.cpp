class NumArray {
    ...
    int buildSegTree(const vector<int> &nums, int left, int right, int node) {
        int middle;
        ...
        middle = (left + right) / 2;
        leftSum = buildSegTree(nums, left, middle, 2 * node + 1);
        rightSum = buildSegTree(nums, middle+1, right, 2 * node + 2);
